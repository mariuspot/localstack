IMAGE_NAME ?= localstack/localstack-multiarchtests
IMAGE_NAME_LIGHT ?= localstack/localstack-multiarchtests-light
IMAGE_NAME_FULL ?= localstack/localstack-multiarchtests-full
...
docker-push-master: 	  ## Push Docker image to registry IF we are currently on the master branch
	# TODO re-add branch and version check
